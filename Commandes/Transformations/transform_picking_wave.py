...
def transform_picking_wave(engine) -> pd.DataFrame:
    ...
    df_wave = df_wave.iloc[:, 0].str.split(";", expand=True)
    df_wave.columns = [
        "waveNumber", "reference", "Size (US)",
        "quantityToPick (units)", "locations", "operator"
    ]

    df_wave_clean = df_wave.drop_duplicates().copy()

    str_cols = ["reference", "locations", "operator"]
    for col in str_cols:
    ...
    df_wave_clean.rename(columns={
        "wavenumber": "wave_number",
        "size_us": "size_us",
        "quantitytopick_units": "quantity_to_pick_units"
    }, inplace=True)
    ...
    df_wave_clean = df_wave_clean[df_wave_clean["quantity_to_pick_units"] >= 0]
    df_wave_clean = df_wave_clean[df_wave_clean["quantity_to_pick_units"] <= 10000]

    df_wave_clean["reference"] = df_wave_clean["reference"].str.upper().str.replace("-", "").str.strip()

    df_wave_clean = df_wave_clean.drop_duplicates(subset=["wave_number", "reference", "locations", "operator"])
